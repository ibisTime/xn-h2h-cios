...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.selectedBtn setImage:[UIImage imageNamed:@"发布_未选中"] forState:UIControlStateNormal];
         [self.selectedBtn setImage:[UIImage imageNamed:@"发布_选中"] forState:UIControlStateSelected];
...
- (void)choose {
    ...
    if (self.photoItem.isSelected == NO ) {
        ...
        if([TLChooseResultManager manager].hasChooseItems.count >= [TLChooseResultManager manager].maxCount) {
            ...
            [TLAlert alertWithInfo:@"最多选择12张图片"];
            return;
...
- (void)setPhotoItem:(TLPhotoChooseItem *)photoItem {
    ...
    if (photoItem.isCamera) {
        ...
        self.photoImageView.image = [UIImage imageNamed:@"添加照片_小"];
