...
@interface CustomDecimalKeyboard()
...
@implementation CustomDecimalKeyboard
...
- (void)_initUI
{
    ...
    for (int i = 0; i < 14; ++i) {
        ...
        if (i == 10) {
            ...
            item.tag = 127;
            [item setImage:kImage(@"delete") forState:UIControlStateNormal];
            ...
            item.tag = -1;
            [item setImage:kImage(@"keyboard") forState:UIControlStateNormal];
        }
        else if (i == 13) {
            item.tag = -2;
        ...
        } else if (item.tag == -1 || item.tag == 127) {
            
            [item setBackgroundColor:kPaleGreyColor forState:UIControlStateNormal];
